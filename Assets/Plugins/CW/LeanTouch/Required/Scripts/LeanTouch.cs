...
namespace Lean.Touch
{
	...
	public partial class LeanTouch : MonoBehaviour
	{
		...
		public static void UnObjeto(Vector2 screenPosition)
		{
			...
			Ray m_ray = Camera.main.ScreenPointToRay(screenPosition);
			RaycastHit m_hit;
			if (Physics.Raycast(m_ray, out m_hit, 1000))
			{
				if (m_hit.collider.gameObject.tag == "objetoMovible")
				{
					...
					foreach (GameObject objeto in objetos)
					{
						...
						{
							objeto.GetComponent<LeanDragTranslate>().enabled = false;
							objeto.GetComponent<Rigidbody>().useGravity = true;
							objeto.GetComponent<Rigidbody>().isKinematic = false;
						}
						else
						{
							if (objetoAgarrado.GetComponent<LeanDragTranslate>().enabled == false)
							{
								objetoAgarrado.GetComponent<LeanDragTranslate>().enabled = true;
